...
function addTaskUI() {
	...
	document.getElementById("taskTimeArea").style.height = "59%";
	document.getElementById("taskTitleArea").style.height = "59%";
}
...
function cancelTaskUI() {
	...
	document.getElementById("addTaskArea").innerHTML = content;
	document.getElementById("taskTimeArea").style.height = "70%";
	document.getElementById("taskTitleArea").style.height = "70%";
}
...
function updateList() {
	...
	for (let i = 0; i < taskList.length; i++) {
		...
		document.getElementById(
			"taskTimeArea"
		).innerHTML += `<div>${time} </div><br>`;
		document.getElementById(
			"taskTitleArea"
		).innerHTML += `<div id="task${i}">${task}<i class="fa fa-trash" style="float:right;" onclick="deleteTask('${i}')"></i></div> <br>`;
	}
}

function loadTasks() {
	if (
		localStorage.getItem(USERTASKKEY) == null ||
		JSON.parse(localStorage.getItem(USERTASKKEY)).length == 0
	) {
		document.getElementById("taskTimeArea").innerHTML = "";
		document.getElementById("taskTitleArea").innerHTML =
